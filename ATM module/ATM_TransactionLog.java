			...
			if (acc_no == _acc_no) {
				if (acc_type=="checking" && type=="withdraw")
						buffer.append(fields[0]+ "\t" + fields[1]+ "\t" + fields[2] + "\t" + fields[3]+
								   "\t" +  fields[4]+ "\t" +  this.balance+ "\t" + fields[6]+ "\t" + _acc_no + "\t" + fields[8] + "\t" + fields[9] +
									System.lineSeparator());
				else if (acc_type=="savings" && type=="withdraw")
					buffer.append(fields[0]+ "\t" + fields[1]+ "\t" + fields[2] + "\t" + fields[3]+
							"\t" +  fields[4]+ "\t" + 0.0 + "\t" + (Double.parseDouble(fields[6])+savings_augment) + "\t" +  _acc_no + "\t" + fields[8] + "\t" + fields[9] +
							System.lineSeparator());

				else if (acc_type=="checking" && type=="deposit")
					buffer.append(fields[0]+ "\t" + fields[1]+ "\t" + fields[2] + "\t" + fields[3]+
							"\t" +  fields[4]+ "\t" + this.balance  + "\t" + Double.parseDouble(fields[6])+ "\t" +  _acc_no + "\t" + fields[8] + "\t" + fields[9] +
							System.lineSeparator());
				else if (acc_type=="savings" && type=="deposit")
					buffer.append(fields[0]+ "\t" + fields[1]+ "\t" + fields[2] + "\t" + fields[3]+
							"\t" +  fields[4]+ "\t" + Double.parseDouble(fields[5]) + "\t" + this.balance + "\t" +  _acc_no + "\t" + fields[8] + "\t" + fields[9] +
							System.lineSeparator());
